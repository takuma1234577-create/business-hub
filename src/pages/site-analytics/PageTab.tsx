...
export function PageSelector({ pages, value, onChange }: { pages: PageRow[]; value: string; onChange: (p: string) => void }) {
  ...
  return (
    <div className="flex flex-wrap items-center gap-2">
      ...
      >
        {!inList && <option value="">{prettyPath(value)}（期間内のアクセスなし）</option>}
        {pages.map((p) => (
          <option key={p.path} value={p.path}>
            {(p.title || prettyPath(p.path)).slice(0, 40)} {prettyPath(p.path).slice(0, 50)}（{fmtNum(p.pageviews)} PV）
          </option>
        ))}
      </select>
      <form
      ...
      >
        <input
          ...
          placeholder="URLかパスを入力（例：/products/xxx）"
        ...
        />
        <button className="text-xs px-3 py-2 rounded-lg bg-slate-900 text-white dark:bg-white dark:text-slate-900 cursor-pointer">表示</button>
      </form>
    </div>
  )
}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex gap-2">
          ...
          <button onClick={onOpenHeatmap} className="inline-flex items-center gap-1.5 text-xs px-3 py-2 rounded-lg bg-slate-900 text-white dark:bg-white dark:text-slate-900 cursor-pointer">
            <Flame size={13} /> ヒートマップを見る
          </button>
        </div>
      </div>

      {err && <p className="text-xs text-rose-600">{err}</p>}
      {!data ? <Empty>{loading ? '集計中…' : 'データがありません'}</Empty> : (() => {
        const k = data.kpis
        ...
        return (
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-8 gap-3">
              ...
              <Kpi label="平均閲覧時間" value={fmtDur(k.avg_time_on_page_sec)} cur={k.avg_time_on_page_sec} prev={p.avg_time_on_page_sec} hint="このページを画面に表示していた時間の平均" />
              <Kpi label="平均スクロール到達" value={fmtPct(k.avg_scroll)} cur={k.avg_scroll} prev={p.avg_scroll} />
              <Kpi label="入口になった回数" value={fmtNum(k.entries)} cur={k.entries} prev={p.entries} />
              <Kpi label="直帰率" value={fmtPct(k.bounce_rate, 1)} cur={k.bounce_rate} prev={p.bounce_rate} lowerIsBetter hint="このページから入り、他のページを見ずに離脱した割合" />
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              <Card title="スクロール到達率（どこまで読まれたか）">
                <ScrollReach data={data.scroll_reach} />
              </Card>
              <Card title="直前に見ていたページ">
                <BarList items={data.prev_pages as unknown as Record<string, unknown>[]} valueKey="count" unit="回" onClickItem={(n) => n.startsWith('/') && onChangePath(n)} />
              </Card>
              <Card title="次に見たページ">
                <BarList items={data.next_pages as unknown as Record<string, unknown>[]} valueKey="count" unit="回" onClickItem={(n) => n.startsWith('/') && onChangePath(n)} />
              </Card>
            </div>
