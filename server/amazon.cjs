...
const ORDER_MAX_AGE_DAYS = 7;            // これより古い注文は自動出荷せず要確認に回す（既出荷の二重出荷防止）
...
router.get('/cron/sync', async (req, res) => {
  ...
  try {
    ...
    let fulfilled = 0, skippedUnmapped = 0, failed = 0, stockWait = 0, alreadyOnAmazon = 0, alreadyFulfilled = 0, needsReview = 0;
    const ageCutoff = Date.now() - ORDER_MAX_AGE_DAYS * 86400000;
    for (const order of pendingOrders || []) {
      // ERROR注文はバックオフ期間が経過してから再試行（即時連打を防ぐ）
      if (order.status === 'ERROR' && order.updated_at && order.updated_at > retryThreshold) continue;

      // 経過日数ガード: 古い注文は既に別経路で出荷済みの可能性が高いため自動出荷せず要確認に回す（二重出荷防止）
      const placedAt = new Date(order.ordered_at || order.created_at).getTime();
      if (placedAt && placedAt < ageCutoff) {
        await supabase.from('orders').update({ status: 'NEEDS_REVIEW', error_message: `[要確認] ${ORDER_MAX_AGE_DAYS}日以上前の注文のため自動出荷を保留（既出荷の可能性）`, updated_at: new Date().toISOString() }).eq('id', order.id);
        await supabase.from('fulfillment_logs').insert({ order_id: order.id, event: 'NEEDS_REVIEW', message: `[自動] ${ORDER_MAX_AGE_DAYS}日以上前の注文のため自動出荷を保留（要手動確認）` });
        needsReview++;
        continue;
      }
    ...
    results.autoFulfill = { fulfilled, skippedUnmapped, failed, stockWait, alreadyOnAmazon, alreadyFulfilled, needsReview };
    console.log(`[cron/sync] Auto-fulfilled ${fulfilled} (unmapped=${skippedUnmapped}, failed=${failed}, stockWait=${stockWait}, alreadyOnAmazon=${alreadyOnAmazon}, alreadyFulfilled=${alreadyFulfilled}, needsReview=${needsReview})`);
    if (needsReview > 0) {
      alerts.push(`${ORDER_MAX_AGE_DAYS}日以上前の古い注文が ${needsReview} 件あり自動出荷を保留しました（既出荷の可能性・要手動確認）`);
    }
